<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF–8">
        <title>Latihan 2 – Variable dan Konstansa</title>
    </head>
    <body>
        <h1>Belajar Variable dan Konstansa di PHP</h1>

        <?php
        // Latihan 4: Menampilkan 7 nama hari dengan mekanisme variabel

        $hari1 = "Senin";
        $hari2 = "Selasa";
        $hari3 = "Rabu";
        $hari4 = "Kamis";
        $hari5 = "Jumat";
        $hari6 = "Sabtu";
        $hari7 = "Minggu";

        echo "<h2>7 Nama Hari</h2>";
        echo "Hari ke-1: $hari1<br>";
        echo "Hari ke-2: $hari2<br>";
        echo "Hari ke-3: $hari3<br>";
        echo "Hari ke-4: $hari4<br>";
        echo "Hari ke-5: $hari5<br>";
        echo "Hari ke-6: $hari6<br>";
        echo "Hari ke-7: $hari7<br>";
        ?>
    </body>
</html>